...
func (s *NFTService) GetNftCollectionInfo(ctx context.Context, req *v1.GetNftCollectionInfoReq) (
	*v1.GetNftCollectionInfoReply, error) {
	// 设置接口 3s 超时
	subctx, cancel := context.WithTimeout(ctx, 3*time.Second)
	defer cancel()
	return s.uc.GetNftCollectionInfo(subctx, req.Chain, req.Address)
}
